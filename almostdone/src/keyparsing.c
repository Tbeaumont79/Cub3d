...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/23 14:15:50 by thbeaumo          #+#    #+#             */
/*   Updated: 2020/02/23 17:27:36 by thbeaumo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		keypress(int keyval, void *param)
{
	...
	datas->game.key[key_up] = keyval == 13 ?
	keyval : datas->game.key[key_up];
	datas->game.key[key_down] = keyval == 1 ?
	keyval : datas->game.key[key_down];
	datas->game.key[key_right] = keyval == 2 ?
	keyval : datas->game.key[key_right];
	datas->game.key[key_left] = keyval == 0 ? 1 : datas->game.key[key_left];
	datas->game.key[rot_left] = keyval == 123 ?
	keyval : datas->game.key[rot_left];
	datas->game.key[rot_right] = keyval == 124 ?
	keyval : datas->game.key[rot_right];
